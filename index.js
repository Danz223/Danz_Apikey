...
app.get('/api/tiktoks', async(req, res) => {
  let q = req.query.q
  ({ status : false, creator : `${creator}`, message : "[!] masukan parameter teks"})  
  let result = await ptz.tiktoks(q)
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/txtstyle', async(req, res) => {
  let text = req.query.text
  ({ status : false, creator : `${creator}`, message : "[!] masukan parameter teks"})  
  let result = await ptz.txtstyle(text)
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/ffstalk', async(req, res) => {
  ...
  if (!gameId) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter id game"})  
  let results = await ptz.ffstalk(gameId)
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/gitstalk', async(req, res) => {
  let user = req.query.user
  if (!user) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter username"})  
  let hasil = await ptz.gitstalk(user)
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/igstory', async(req, res) => {
  let url = req.query.url
  if (!url) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})  
  let hasil = await ptz.igstory(url)
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/HariLibur', async(req, res) => {
  let result = await ptz.HariLibur()
  try {
res.json({
  status: 200,
  creator: "Danz",
...
app.get('/api/mediafire', async(req, res) => {
  let url = req.query.url
  if (!url) return res.json({ status : false, creator : `${creator}`, message : "[!] masukan parameter url"})  
  let result = await ptz.mediafire(url)
  try {
  res.json({
    status: 200,
    creator: "Danz",
